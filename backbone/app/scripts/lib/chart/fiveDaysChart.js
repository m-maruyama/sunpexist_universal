define([
	 ...
	 * construtor
	 */
	var FiveDaysChart = function(anyOptions){
		...
		this.m_options = {
			...
			charts: {
				...
				noData: {//休場
					color_cd:[0,0,0,0],	//線の色,透明度
					fill: true,
					//fillColor: [255,255,255,0.2],
					fillColor: [35,49,87,1],
					width:1,				  //線の幅
					shadowSize:false,		 //flotr2のデフォルトの影非表示
					shadowColor:[0,0,0,1],	//影の色
					shadowBlur:0,			 //ぼかしの大きさ
	...
	FiveDaysChart.prototype = {
		...
		draw: function(data){
			...
			var i, nTime,sTime, time, nLength, h, d;
			var now = new Date();
			...
			if (this.n_px in data) {
				...
				sTime = now.setHours(9,0,0,0);
				for (i = 0; i < nLength; i++) {	//価格
					nTime = data.px[i].time;
					h = utils.getHH(nTime);
					d = Math.floor(h/24);
					h = h % 24;
					time = now.setHours(h, utils.getMM(nTime), utils.getSS(nTime),0) + d * 60 * 60 * 24 * 1000;
					this.d_px.push([time, data.px[i].val, data.px[i].color_cd]);
				}
			}

			if (this.n_noData in data) {
				nLength = data.noData.length;
				sTime = now.setHours(9,0,0,0);
				for (i = 0; i < nLength; i++) {	//価格
					nTime = data.noData[i].time;
					h = utils.getHH(nTime);
					d = Math.floor(h/24);
					h = h % 24;
					time = now.setHours(h, utils.getMM(nTime), utils.getSS(nTime),0) + d * 60 * 60 * 24 * 1000;
					this.d_noData.push([time, data.noData[i].val]);
				}
			}
			if (this.n_date in data) {
				nLength = data.date.length;
				sTime = now.setHours(9,0,0,0);
				for (i = 0; i < nLength; i++) {
					nTime = data.date[i].time;
					h = utils.getHH(nTime);
					d = Math.floor(h/24);
					h = h % 24;
					time = now.setHours(h, utils.getMM(nTime), utils.getSS(nTime),0) + d * 60 * 60 * 24 * 1000;
					this.d_date.push([time, data.date[i].val]);
				}
			}
			...
			//X軸固定用ダミーデータ
			var dummyTime = stime;
			for(dummyTime;dummyTime<=etime + (60 * 60 * 1000 * 4 * 6);dummyTime+=300000){
			...
			this.hourTicks = [];
			for(var t=0;t<=15;t++){
				this.hourTicks.push(stime + t * 60 * 60 * 1000 * 2);
				this.hourTicks.push(stime + t * (60 * 60 * 1000 + 30 * 60 * 1000) * 2);
			}
		...
		_drawGraph: function() {
			...
			if (true) {	 //休場
				gDataSet.push({
					...
					lines: {
						show: true,
						color_cd: utils.convertToRgba(this.m_options.charts.noData.color_cd),
						fillOpacity: utils.getFillOpacity(this.m_options.charts.noData.fillColor),
						fillColor:utils.convertToRgba(this.m_options.charts.noData.fillColor),
						fill:this.m_options.charts.noData.fill,
						lineWidth: this.m_options.charts.noData.width,
						shadowSize: this.m_options.charts.noData.shadowSize,
						shadowColor: utils.convertToRgba(this.m_options.charts.noData.shadowColor),
						shadowBlur: this.m_options.charts.noData.shadowBlur,
						shadowOffsetX: this.m_options.charts.noData.shadowOffsetX,
						shadowOffsetY: this.m_options.charts.noData.shadowOffsetY
					},
					yaxis: 2
				});
			}
			...
			if (this.f_px) {	//価格
				gDataSet.push({
					...
					lines: {
						...
						colorful: this.m_options.charts.px.colorful
					}
				});
			...
			if (this.m_options.yaxis2.showLabels){
				tickFormatter = function(n) {
					...
					h = h - 9;
					h = h - Math.floor(h / 6) * 6;
					h = h + 9;
					var hh = ("0"+ h).slice(-2);  // 1桁の数字を0埋めで2桁にする
			...
			var gOptionsSet = {
				...
				xaxis:{
					...
					max: (new Date()).setHours(15,10,0,0) + 60 * 60 * 1000 * 4 * 6
				},
				yaxis: {
					ticks: yticks,
					showLabels: true,
				...
				mouse : {
					...
					trackFormatter: function (o) {
						...
						var dateTxt = String(_.find(that.d_date, function(value, key){
							return value[0] === tTime;
						})[1]);
						var weekday = ['日','月','火','水','木','金','土'];
						var year = dateTxt.substr(0,4);
						var month = dateTxt.substr(4,2);
						var date = dateTxt.substr(6,2);
						var day = weekday[(new Date(parseInt(year,10), parseInt(month,10) - 1, parseInt(date,10))).getDay()];

						html  += '<tr><td>日付</td><td>' + dateTxt.substr(0,4) + '.' + dateTxt.substr(4,2)+ '.' + dateTxt.substr(6,2) + "("+day+")"; + '</td></tr>';

						var sTime = ("0"+ h).slice(-2) + ':' + ("0"+ d1.getMinutes()).slice(-2);
			...
			// Graph描画
			/* global Flotr */
			return flotr2.draw(this.m_options.target.get(0), gDataSet, gOptionsSet, this.data, this);
		},
		mousemove: function(){
