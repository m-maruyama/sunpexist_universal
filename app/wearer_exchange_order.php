 ...
 *
 */
$app->post('/wearer_exchange/info', function ()use($app){
  ...
  if (!empty($results_cnt)) {
    foreach ($results as $result) {
      $list["return_date"] = $result->return_date;
    }
  }
  $sex_kbn = '';
  // 着用者基本マスタトラン参照
  ...
  if (!empty($results_cnt)) {
    ...
    if (empty($all_list)) {
        ...
        if (!empty($results_cnt)) {
            ...
            $query_list[] = "m_wearer_std.rntl_cont_no = '".$wearer_size_change_post['rntl_cont_no']."'";
            $query_list[] = "m_wearer_std.werer_cd = '".$wearer_size_change_post['werer_cd']."'";
            $query = implode(' AND ', $query_list);
  ...
  if (!empty($cond["job_type_cd"])) {
    $query_list[] =  "job_type_cd = '".$cond['job_type_cd']."'";
  } else {
    $query_list[] =  "job_type_cd = '".$wearer_size_change_post['job_type_cd']."'";
  }
  $query = implode(' AND ', $query_list);
  $arg_str = '';
  $arg_str .= 'SELECT ';
  ...
  //ChromePhp::LOG($order_control_unit);
    ...
    $query_list[] =  "relation_cls_cd = '001'";
    $query_list[] =  "relation_gen_cd = '3'";
    $query_list[] =  "relation_cls_cd_2 = '003'";
    $query_list[] =  "relation_gen_cd_2 = '".$order_control_unit."'";
    $query = implode(' AND ', $query_list);
  ...
  $query_list[] = "corporate_id = '".$auth['corporate_id']."'";
  $query_list[] = "rntl_cont_no = '".$wearer_size_change_post['rntl_cont_no']."'";
  $query_list[] = "rntl_sect_cd = '".$wearer_size_change_post['rntl_sect_cd']."'";
  $query = implode(' AND ', $query_list);
  ...
  //--発注情報トラン・返却予定情報トラン内、「サイズ交換」情報の有無確認--//
  //※発注情報トラン参照
  $query_list = array();
  $query_list[] = "t_order_tran.corporate_id = '".$auth['corporate_id']."'";
  $query_list[] = "t_order_tran.rntl_cont_no = '".$wearer_size_change_post['rntl_cont_no']."'";
 ...
 * 入力項目：発注商品一覧
 */
$app->post('/wearer_exchange/list', function ()use($app){
  $params = json_decode(file_get_contents("php://input"), true);
  ...
  // 発注情報トラン(サイズ交換発注履歴)確認
  $order_tran_list = array();
  $list = array();
  $query_list = array();
  $query_list[] = "m_wearer_std_tran.corporate_id = '".$auth['corporate_id']."'";
  $query_list[] = "m_wearer_std_tran.rntl_cont_no = '".$wearer_size_change_post['rntl_cont_no']."'";
  ...
  $arg_str .= " * ";
  $arg_str .= " FROM ";
  $arg_str .= "(SELECT distinct on (t_order_tran.item_cd, t_order_tran.color_cd, t_order_tran.size_cd) ";
  ...
  // 返却予定情報トラン(サイズ交換発注履歴)確認
  $return_tran_list = array();
  $list = array();
  $query_list = array();
  $query_list[] = "m_wearer_std_tran.corporate_id = '".$auth['corporate_id']."'";
  $query_list[] = "m_wearer_std_tran.rntl_cont_no = '".$wearer_size_change_post['rntl_cont_no']."'";
...
//              }
//              //数量 - 返却予定数を交換可能枚数とする
//              $list["exchange_possible_num"] = $item_result->as_quantity - $item_result->as_return_plan_qty;
          }
        // name属性用カウント値
        ...
        if (individual_flg($auth['corporate_id'], $wearer_size_change_post['rntl_cont_no']) == "1") {
          ...
          $list["individual_ctrl_no"] = implode("<br/>", $list["individual_ctrl_no"]);
            ...
            $list["exchange_possible_num"] = $each_item_quantity - $each_item_return_plan_qty;
            //交換可能枚数が0以下だったら表示しない
            if($list["exchange_possible_num"] <= 0){
                $arr_num--;
                $list_cnt--;
                continue;
            }
...
//                    // 所持枚数
//                    $list["exchange_possible_num"] = $list["exchange_possible_num"] - $t_returned_tran_item->return_plan_qty;
//                }
//                //ChromePhp::log($list["possible_num"]);
//                if ($list["exchange_possible_num"] < 1) {
//                    $arr_num--;
//                    $list_cnt--;
//                    continue;
//                }
...
//            }
        ...
        $list["return_num"] = 0;
        if (individual_flg($auth['corporate_id'], $wearer_size_change_post['rntl_cont_no']) == "0") {
          if ($item_result->as_size_add_flg == "0") {
...
//              }
//              //数量 - 返却予定数を交換可能枚数とする
//              $list["exchange_possible_num"] = $result->as_quantity - $result->as_return_plan_qty;
          }
        // name属性用カウント値
        $list["arr_num"] = $arr_num++;
        // No
  ...
  if (!empty($all_list)) {
    foreach ($all_list as $all_map) {
      if (!empty($all_map["return_num"])) {
        $list["sum_return_num"] += $all_map["return_num"];
      }
    }
  }
 ...
 * 発注取消処理
 */
$app->post('/wearer_exchange/delete', function ()use($app){
  $params = json_decode(file_get_contents("php://input"), true);
...
//    }
    ...
    array_push($query_list, "t_order_tran.order_req_no = '".$cond['order_req_no']."'");
    // 発注区分「サイズ交換」
    array_push($query_list, "t_order_tran.order_sts_kbn = '3'");
    $query = implode(' AND ', $query_list);
    ...
    // 発注区分「サイズ交換」
    array_push($query_list, "t_returned_plan_info_tran.order_sts_kbn = '3'");
    $query = implode(' AND ', $query_list);
         ...
         if (!$item_map["individual_flg"]) {
             ...
             if(isset($item_map["size_cd"])){
                 ...
                 if($t_order_tran_count > 0) {
                     foreach ($t_returned_tran_items as $t_returned_tran_item) {
                         // 所持枚数
                         $item_map["exchange_possible_num"] = $item_map["exchange_possible_num"] - $t_returned_tran_item->return_plan_qty;
                     }
...
//       }
     ...
     if (!empty($results_cnt)) {
       ...
       foreach ($results as $result) {
         ...
         // 発注情報トラン.発注状況区分 = 「貸与（追加貸与）」「終了」または「異動」情報がある際は発注NG
...
//                           if ($t_rtn_tran_count > 0) {
//                               $json_list["error_code"] = "1";
//                               $error_msg = $item_map['item_cd'] . "-" . $item_map['color_cd'] . "の個体管理番号:" . $individual_data['individual_ctrl_no'] . "は既にその他交換の発注がされています。";
//                               array_push($json_list["error_msg"], $error_msg);
//                           }
//                       }
//                   }
...
//       }
     ...
     array_push($query_list, "order_sts_kbn = '3'");//サイズ交換
     $query = implode(' AND ', $query_list);
     ...
     try {
       ...
       if ($tran_results_cnt > 0) {
         ...
         // 発注状況区分(サイズ交換)
         if ($order_sts_kbn !== "6") {
           array_push($up_query_list, "order_sts_kbn = '3'");
         }
         // 更新区分(WEB発注システム(サイズ交換）)
         array_push($up_query_list, "upd_kbn = '3'");
         // Web更新日時
      ...
      if (!empty($item_list)) {
        ...
        foreach ($item_list as $item_map) {
          ...
          if ($item_map["individual_flg"] == true && !empty($item_map["individual_data"])) {
            ...
            array_push($query_list, "t_delivery_goods_state_details.size_cd = '".$item_map['now_size_cd']."'");
 ...
 * 発注送信処理
 */
$app->post('/wearer_exchange/send', function ()use($app){
  $params = json_decode(file_get_contents("php://input"), true);
...
//                  foreach ($item_map["individual_data"] as $individual_data) {
//
//                      if ($individual_data["return_num"] > 0) {
//                          $query_list = array();
//                          array_push($query_list, "corporate_id = '" . $auth['corporate_id'] . "'");
//                          array_push($query_list, "rntl_cont_no = '" . $wearer_size_change_post['rntl_cont_no'] . "'");
//                          array_push($query_list, "werer_cd = '" . $wearer_size_change_post['werer_cd'] . "'");
