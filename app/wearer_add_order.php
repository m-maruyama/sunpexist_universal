 ...
 */
 $app->post('/wearer_add_list', function ()use($app){
   ...
   $arg_str .= " FROM ";
   $arg_str .= "m_job_type INNER JOIN m_wearer_std_tran";
   $arg_str .= " ON (m_job_type.corporate_id = m_wearer_std_tran.corporate_id";
   $arg_str .= " AND m_job_type.rntl_cont_no = m_wearer_std_tran.rntl_cont_no";
   $arg_str .= " AND m_job_type.job_type_cd = m_wearer_std_tran.job_type_cd)";
     ...
     foreach ($results as $result) {
       ...
       if (!empty($results_cnt)) {
         ...
         foreach ($m_item_results as $m_item_result) {
           array_push($list["size_cd"], $m_item_result->size_cd);
         }
       }
       // 発注枚数
...
/*
       // 返却枚数フォーマットチェック
       if (empty($item_map["return_num_disable"])) {
...
*/
    ...
    // 現在貸与中のアイテム
    foreach ($item_input as $item_input_map) {
      // 発注枚数フォーマットチェック
      if (empty($item_input_map["order_num_disable"])) {
        if (!ctype_digit(strval($item_input_map["order_num"]))) {
          if (empty($order_num_format_err)) {
            $order_num_format_err = "err";
            $json_list["error_code"] = "1";
      ...
      // 返却枚数フォーマットチェック
      if (empty($item_input_map["return_num_disable"])) {
        if (!ctype_digit(strval($item_input_map["return_num"]))) {
          if (empty($return_num_format_err)) {
            $return_num_format_err = "err";
            $json_list["error_code"] = "1";
            $error_msg = "現在貸与中のアイテムの返却枚数には半角数字を入力してください。";
      ...
      // 返却枚数チェック
      if (!empty($item_input_map["individual_data"])) {
        $target_cnt = 0;
        for ($i=0; $i<count($item_input_map["individual_data"]); $i++) {
          if ($item_input_map["individual_data"][$i]["target_flg"] == "1") {
            $target_cnt = $target_cnt + 1;
          }
        }
        if ($item_input_map["return_num"] != $target_cnt) {
          if (empty($return_num_err)) {
            $return_num_err = "err";
            $json_list["error_code"] = "1";
...
/*
      // 現在貸与中のアイテム内容登録
      if (!empty($item_input)) {
        foreach ($item_input as $item_map) {
          $calum_list = array();
          $values_list = array();
...
*/
      ...
      if (!empty($item_input)) {
        if ($wearer_other_post['order_tran_flg'] == '1') {
          $query_list = array();
          array_push($query_list, "corporate_id = '".$auth['corporate_id']."'");
          array_push($query_list, "order_req_no = '".$wearer_other_post['order_req_no']."'");
          // 発注区分「異動」
     ...
     if (!empty($item_input)) {
       ...
       if (!empty($wearer_other_post['return_req_no'])) {
         $query_list = array();
         array_push($query_list, "corporate_id = '".$auth['corporate_id']."'");
         array_push($query_list, "order_req_no = '".$wearer_other_post['return_req_no']."'");
         // 発注区分「異動」
         array_push($query_list, "order_sts_kbn = '5'");
         $query = implode(' AND ', $query_list);
       ...
       foreach ($item_input as $item_map) {
         ...
         if (!empty($item_map["return_num"])) {
           if (!empty($item_map["individual_data"])) {
             // 個体管理番号、対象単位での登録処理
             foreach ($item_map["individual_data"] as $individual_data_map) {
               // 個体管理番号単位で対象にチェックがONのデータのみ登録
               if ($individual_data_map["target_flg"] == "1") {
                 $calum_list = array();
                 $values_list = array();
