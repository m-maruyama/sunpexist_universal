 ...
 * 検索一覧
 */
$app->post('/wearer_other/search', function ()use($app){
  ...
  $query_list[] = "wjt.add_and_rtn_rntl_flg = '1'";
 ...
 */
$app->post('/wearer_other/req_param', function ()use($app){
  $params = json_decode(file_get_contents("php://input"), true);
  ...
  // POSTパラメータのセッション格納
  $app->session->set("wearer_other_post", array(
  ...
  $json_list = array();
  $json_list = $app->session->get("wearer_other_post");
