...
//	$params = json_decode(file_get_contents("php://input"), true);
  ...
  $page = $params['page'];
    ...
    if ($cond["ui_type"] === "history") {
        ...
        $arg_str .= " FROM t_order ";
        $arg_str .= " LEFT JOIN t_delivery_goods_state_details";
        $arg_str .= " ON t_delivery_goods_state_details.corporate_id = t_order.corporate_id";
        $arg_str .= " AND t_delivery_goods_state_details.order_req_no = t_order.order_req_no";
        $arg_str .= " AND t_delivery_goods_state_details.item_cd = t_order.item_cd";
        $arg_str .= " AND t_delivery_goods_state_details.color_cd = t_order.color_cd";
        $arg_str .= " AND t_delivery_goods_state_details.size_cd = t_order.size_cd";
        $arg_str .= " LEFT JOIN (t_order_state LEFT JOIN t_delivery_goods_state";
        ...
        if (!empty($results_cnt)) {
            ...
            foreach ($results as $result) {
                ...
                //---個体管理番号・受領日時の取得---//
                $list['individual_num'] = $result->as_individual_ctrl_no;
                if(!empty($result->as_receipt_date)){
                    $list['order_res_ymd'] = $result->as_receipt_date;
                }else{
                    $list['order_res_ymd'] = '-';
                }

                //個ありの出荷数、発注数、受注数、個数対策 必ず1をセット
                if (individual_flg($auth['corporate_id'], $cond['agreement_no']) == 1) {
                    $list['ship_qty'] = '1';
                    $list['order_qty'] = '1';
                }
        ...
        */
        ...
        array_push($header_2, "店舗コード");
        array_push($header_2, "貸与パターン");
        array_push($header_2, "社員番号");
        array_push($header_2, "着用者名");
        array_push($header_2, "商品-色");
        array_push($header_2, "サイズ");
        array_push($header_2, "サイズ2");
        ...
        if (!empty($all_list)) {
            foreach ($all_list as $all_map) {
                ...
                // 店舗コード
                array_push($csv_body_list, $all_map["rntl_sect_cd"]);
                // 貸与パターン
                ...
                // サイズ
                array_push($csv_body_list, $all_map["size_cd"]);
                // サイズ2
                array_push($csv_body_list, $all_map["size_two_cd"]);
                // 商品-色(サイズ-サイズ2)・商品名
...
//            array_push($reason_kbn_4, "t_order.order_reason_kbn = '24' AND m_wearer_std.werer_sts_kbn = '1'");
      ...
      if ($reason_kbn_4) {
        ...
        if (individual_flg($auth['corporate_id'], $cond['agreement_no']) == 1) {
            ...
            $arg_str .= " FROM ";
//	$arg_str .= "(SELECT ";
            $arg_str .= "(SELECT distinct on (t_returned_plan_info.order_req_no, t_returned_plan_info.individual_ctrl_no) ";
...
//             $list['return_date'] = $return_date;
             ...
             //---個体管理番号・受領日時の取得 CSV用１個ずつ---//
             $list['individual_num'] = $result->as_individual_ctrl_no;
             if(!empty($result->as_return_date)){
                 $list['return_date'] = $result->as_return_date;
             }else{
                 $list['return_date'] = '-';
             }
         }
...
//				array_push($csv_body_list, $all_map["ship_qty"]);
    ...
    if ($cond["ui_type"] === "receive") {
        ...
        if (individual_flg($auth['corporate_id'], $cond['agreement_no']) == 1) {
            ...
            $arg_str .= "t_delivery_goods_state_details.individual_ctrl_no) ";
        } else {
            $arg_str .= "(SELECT distinct on ";
...
//                }
    ...
    if ($cond["ui_type"] === "lend") {
        ...
        $arg_str .= " FROM ";
        if (individual_flg($auth['corporate_id'], $cond['agreement_no']) == 1) {
            $arg_str .= "(SELECT distinct on (t_delivery_goods_state_details.order_req_no,t_delivery_goods_state_details.individual_ctrl_no) ";
        } else {
...
//                    $list['order_kbn'] = $order_kbn;
                    if (individual_flg($auth['corporate_id'], $cond['agreement_no']) == 1) {
                        ...
                        $list['quantity'] = '1';
                        //返却予定数
                        $list['return_plan__qty'] = $result->as_return_plan__qty;
                        //発注no
                        $list['order_req_no'] = $result->as_order_req_no;
                        // 現在の拠点
                        $search_q = array();
                        ...
                        ->execute();
                        foreach ($gencode as $gencode_map) {
                            $list['order_kbn'] = $gencode_map->gen_name;
                        }
        ...
        array_push($header_2, "貸与数");
        array_push($header_2, "出荷日");
        array_push($header_2, "返却予定数");
        array_push($header_2, "発注No");
        array_push($header_2, "発注区分");
        array_push($header_2, "納品時の拠点");
        array_push($header_2, "納品時の店舗コード");
        ...
        if (!empty($all_list)) {
            foreach ($all_list as $all_map) {
                ...
                // 現在の店舗コード　
                array_push($csv_body_list, $all_map["now_rntl_sect_cd"]);
                // 現在の貸与パターン
                ...
                // 納品時の拠点
                array_push($csv_body_list, $all_map["old_rntl_sect_cd"]);
                // 納品時の貸与パターン
